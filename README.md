...
L'adapteur par defaut est l'adapteur web mais dans un context cordova, il faut définir un adapteur particulier qui s'appuie sur le plugin Cordova [Media](https://cordova.apache.org/docs/en/9.x/reference/cordova-plugin-media/index.html). Ce plugin doit être ajouté au projet.

```bash
cordova plugin add cordova-plugin-media
```

Une fois le plugin installé, il faut indiquer l'utilisation de l'adapter au niveau de tous les éléments

```html
<staytuned-mobile-sdk-player adapter-type="cordova" />
<staytuned-widget adapter-type="cordova" />
<staytuned-mobile-sdk-content-view adapter-type="cordova" />
<staytuned-mobile-sdk-element-view adapter-type="cordova" />
```

Enfin, ajouter le fichier cordova-adapter.js dans la page principale

```html
<script src="%PUBLIC_URL%/cordova-adapter.js" ></script>
```
